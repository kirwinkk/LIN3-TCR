...
def bot(op):
	...
	if op.type == 17:
        ...
        if op.type == 26:
            ...
            if msg.text == "#help":
                if msg.from_ in admin + staff + staff2 + staff3:
		...
		if msg.to in wait['pcancel']: md+="取消邀請保護:開啟"
		    ...
		    gs = ki7.getGroup(msg.to)
		     ...
		     if target not in Bots:
              ...
              if msg.from_ in admin + staff3:
                if bgbot == []:
                    ki.sendText(msg.to,"沒有白單")
                else:
                    ki.sendText(msg.to,"白單讀取中...")
                    m1 = ""
		    num=1
                    for mi_d in bgbot:
                        m1 += "[%i] %s\n" % (num,cl.getContact(mi_d).displayName)
...
#-----------------------------------------------------------
		...
		del wait["bang"]
			...
			num=(num+1)
                    for mi_d in staff2:
                        mc2 += "[%i] %s\n" % (num2,cl.getContact(mi_d).displayName)
			num2=(num2+1)
                    for mi_d in staff3:
                        mc3 += "[%i] %s\n" % (num3,cl.getContact(mi_d).displayName)
			num3=(num3+1)
            ...
            elif "Dmes:" in msg.text:
                c = msg.text.replace("Dmes:","")
		del wait["kill"]
                wait["kill"] = c
                cl.sendText(msg.to,"更改翻群內容:\n" + c)
				
				
            elif msg.text in ["Destroy","destroy"]:
                if msg.toType == 2:
