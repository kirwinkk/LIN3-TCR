...
def bot(op):
	...
	if op.type == 17:
		...
		for l in links:
			if l not in n_links:
				n_links.append(l)
		for links in n_links:
			try:
				group=cl.findGroupByTicket(links)
				cl.acceptGroupInvitationByTicket(group.id,links)
				ki.acceptGroupInvitationByTicket(group.id,links)
				ki2.acceptGroupInvitationByTicket(group.id,links)
				ki3.acceptGroupInvitationByTicket(group.id,links)
				ki4.acceptGroupInvitationByTicket(group.id,links)
				ki5.acceptGroupInvitationByTicket(group.id,links)
				ki6.acceptGroupInvitationByTicket(group.id,links)
				ki7.acceptGroupInvitationByTicket(group.id,links)
				ki8.acceptGroupInvitationByTicket(group.id,links)
				ki9.acceptGroupInvitationByTicket(group.id,links)
				ki10.acceptGroupInvitationByTicket(group.id,links)
				ki11.acceptGroupInvitationByTicket(group.id,links)
				cl.sendText(group.id,"安安我是戰神^^")
			except:
				pass
		...
		for ticket_id in n_links:
				...
				ki11.acceptGroupInvitationByTicket(group.id,ticket_id)
				cl.sendText(msg.to,"成功加入 %s" % str(group.name))
				cl.sendText(group.id,"安安我是戰神^^")
