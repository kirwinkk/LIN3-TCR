...
ki15 = LINETCR.LINE()
ki15.login(token="Elwqog7uvMTEiGRQER69.aLZN+GwDwgQS69XUBHE2Aq.w8FhrJeMLp0AbvHMIzIF2FTMfA9nbPP0gEfcDo19k+U=")
ki15.loginResult()
...
def bot(op):
    try:
        ...
        if op.type == 26:
            msg = op.message
            if msg.text in ["/Kicker","/kicker","kicker","Kicker"]:
		if msg.from_ in ki15mid:
                        G = ki15.getGroup(msg.to)
                        ginfo = ki15.getGroup(msg.to)
                        G.preventJoinByTicket = False
                        ki15.updateGroup(G)
                        invsend = 0
                        Ticket = ki15.reissueGroupTicket(msg.to)
	      ...
	      if msg.from_ in ki15mid:
                if msg.toType == 2:
                    ...
                    if group.invitee is not None:
                        ...
                        ki15.sendText(msg.to,"取消了 "+ str(len(group.invitee)) + " 個邀請")
                    else:
                        if wait["lang"] == "JP":
                            ki15.sendText(msg.to,"找不到能取消的邀請")
                        else:
                            ki15.sendText(msg.to,"找不到能取消的邀請")
                else:
                    pass	    
		...
		gid = ki15.getGroupIdsJoined()
                for i in gid:
                    cl.sendText(i,"戦神公開保護  完全停止!\n若想得到更多新的免費機器消息,請關注作者,謝謝.\n\n戦神Bot作者:戦神 Made In Taiwan\nhttp://line.me/ti/p/4-ZKcjagH0")
                    cl.leaveGroup(i)
                    ki.leaveGroup(i)
                    ki2.leaveGroup(i)
                    ki3.leaveGroup(i)
