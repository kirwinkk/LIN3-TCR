...
cl = LINETCR.LINE()
cl.login(token="Enu7qSuvN4bC40fK2iL3.2XannlnZppIccyjdoNFtaW.Hf5dC72pejJdQwCamM+w+FnkfEOgrJ6Ol1GUgNyJnIw=")
cl.loginResult()
        ...
        if op.type == 13:
           if mid in op.param3:
				...
				if group.invitee is not None:
					...
					try:
						cl.sendText(op.param1,str(len(group.invitee)) + " Done " + random.choice(omikujilist) + "\nBOT作者:戦神↓")
					except:
                        			cl.sendText(op.param1,str(len(group.invitee)) + " Done ヾ(*´∀｀*)ﾉ\nBOT作者:戦神↓")
                    		else:
					a1, a2, a3, a4, a5, a6, a7, b1, b2, b3, b4, b5 = "(*｀＾´)=3", "(`皿´)", "(o｀з’*)", "ヽ(｀Д´)ﾉ", "Σ(´д｀;)","(*'A^q)", "(*´>д<)", "(>_<)", "σ(oдolll)", "( #｀Д´)", "(｀А´)", "(#ﾟДﾟ)"
                			omikujilist = [a1,a2,a3,a4,a5,a6,a7,b1,b2,b3,b4,b5]
                        		cl.sendText(op.param1,"No invite " + random.choice(omikujilist) + "\nBOT作者:戦神↓")
