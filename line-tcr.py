...
def bot(op):
    try:
		...
		kill = cl.getGroup(msg.to)
		killer = kill.members[0].displayName
		if msg.from_ in admin + killer:
		    source_str = 'abcdefghijklmnopqrstuvwxyz1234567890@:;/!&%$#'
		    name = "".join([random.choice(source_str) for x in xrange(9)])
			...
			cl.sendText(msg.to,"只有創群者或是首位入群者能使用此功能\n\n" + datetime.datetime.today().strftime('%H:%M:%S') + " [" + name)
            #elif "gurl" == msg.text:
		...
		if msg.toType == 2:
		 ...
		 name = "".join([random.choice(source_str) for x in xrange(9)])
                 ...
                 try:
                        ...
                        gCreator = ginfo.members[0].displayName
		 cl.sendText(msg.to,"[此群首位入群者(權限者)]\n->" + gCreator + "\n\n" + datetime.datetime.today().strftime('%H:%M:%S') + " [" + name)
