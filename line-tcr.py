...
ki16 = LINETCR.LINE()
ki16.login(token=" El2e4tTG29CjOqKMB0h8.w0WR2OKO/canEpl7cVYAMa.6GMKEhlAD0dzu6xu8x4Wl+fSApjtztJ2saUz+UzRWnc=")
ki16.loginResult()
...
#			    random.choice(KAK).kickoutFromGroup(op.param1,[op.param2])
	...
	if op.type == 19:
		...
		ki15.sendText(op.param1,"別在死神面前踢人^^")
	if op.type == 13:
	    if op.param2 not in Bots:
		if op.param2 in Bots:
		    pass
		    ...
		    ki15.sendText(op.param1,"死神不讓你們邀人^^")
	    else:
	...
	if op.type == 11:
		...
		if op.param2 in Bots:
		    ...
		    ki15.sendText(op.param1,"死神不准你們動網址^^")
	    else:
		ki15.sendText(op.param1,"")
        if op.type == 5:
            if wait["autoAdd"] == True:
